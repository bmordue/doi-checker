...
import { 
  ...
  createErrorResponse, safeJsonParse, withErrorHandling 
} from './lib/errors.js';
import { isValidDOI } from './lib/doi-validator.js';
...
async function checkAllDOIs(env, log) {
  try {
    ...
    log.error('Error in checkAllDOIs', { error: error.message, stack: error.stack });
    throw error;
  }
}
...
async function checkSingleDOI(doi) {
  ...
  try {
    ...
    // Convert fetch errors to a standardized format
    return {
      doi: doi,
      working: false,
      httpStatus: null,
...
async function getStatus(env, log) {
  try {
    ...
    for (const doi of doiList) {
      ...
      try {
        status = statusJson ? JSON.parse(statusJson) : { working: null, lastCheck: null };
      } catch (error) {
        log.warn(`Error parsing status for DOI ${doi}`, { error: error.message });
        status = { working: null, lastCheck: null, error: 'Status data corrupted' };
      }
